...
type AsciiArtProps = {
  ...
  /** Crop/focus anchor, like CSS object-position (e.g. "top", "center", "bottom") */
...
const parseObjectPosition = (
  ...
  const yMap: Record<string, number> = { top: 0, center: 0.5, bottom: 1 };

  if (tokens.length === 1) {
    const token = tokens[0];
    if (token in yMap && token !== "center") {
      return { x: 0.5, y: yMap[token] };
    }
    if (token in xMap) {
      return { x: xMap[token], y: 0.5 };
    }
  }

  return {
    x: xMap[tokens[0]] ?? 0.5,
    y: yMap[tokens[1] ?? "center"] ?? 0.5,
  };
};
...
export const AsciiArt: React.FC<AsciiArtProps> = ({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const { x: xPos, y: yPos } = parseObjectPosition(objectPosition);
  ...
  }, [src, resolution, effectiveCharset, objectFit, objectPosition]);
